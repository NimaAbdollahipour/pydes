...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        for block in text_blocks:  # Loop over all the blocks of data
            ...
            for i in range(16):  # Do the 16 rounds
                ...
                g = d
                d = tmp
            result += self.permut(d + g, PI_1)  # Do the last permute and append the result to result
    ...
    def substitute(self, d_e):  # Substitute bytes using S BOX
        ...
        for i in range(len(subblocks)):  # For all the sublists
            ...
            bin = bin_value(val, 4)  # Convert the value to binary
            result += [int(x) for x in bin]  # And append it to the resulting list
        return result

    def permut(self, block, table):  # Permute the given block using the given table (so generic method)
    ...
    def generate_keys(self):  # Algorithm that generates all the keys
        ...
        key = self.permut(key, CP_1)  # Apply the initial permute on the key
        g, d = nsplit(key, 28)  # Split it in to (g->LEFT),(d->RIGHT)
        for i in range(16):  # Apply the 16 rounds
            g, d = self.shift(g, d, SHIFT[i])  # Apply the shift associated with the round (not always 1)
            tmp = g + d  # Merge them
            self.keys.append(self.permut(tmp, CP_2))  # Apply the permute to get the Ki
